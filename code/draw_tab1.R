...
tmp %>%
  filter(
    pop_source == "YB",
    age_structure_source == "YB",
    process == "lm",
    year == 2018,
    !(CNTY_CODE %in% to_remove)
  ) %>%
  mutate(prv_no = substr(CNTY_CODE, 1, 2),
         region = substr(CNTY_CODE, 1, 1)) %>%
  left_join(labels_region, by = "region") %>%
  group_by(region_label) |> 
  summarise(min = min(cov)*100,
            Q1 = quantile(cov, 0.25)*100,
            mean = mean(cov)*100,
            median = median(cov)*100,
            Q3 = quantile(cov, 0.75)*100,
            max = max(cov)*100,
            sd = sd(cov)*100) |> 
  left_join(data_covered_county, by = "region_label") |> 
  dplyr::select(-location_presence) |> 
  rename(`Sample Size` = data_presence,
         ...
         `Mean (%)` = mean,
         `Median (%)` = median,
         Region = region_label) -> to_save

write_csv(to_save, "table1.csv")
