...
data_coverage %>% 
  ...
  bind_rows(data.frame(before_2019 = c("2019", "2019"),
                       label_region = c("2", "5"),
                       data_absent = c(F, F),
                       county_counts = c(0, 0),
                       county_tot = c(299, 532))) %>% 
  mutate(label_region = factor(label_region,
                               levels = 6:1,
                               labels = rev(labels_region$names_region)),
         before_2019 = factor(before_2019,
                              levels = c("Before 2019",
                                         "2019"))) %>% 
...
plot_grid(p_legend, 
          ...
          rel_heights = c(1,10), 
          ncol = 1) +
  theme(plot.background = element_rect(fill = "white", colour = "white")) -> p_save

ggsave("figs/fig1_v4.png", p_save, width = 10, height = 12)

APC %>% dplyr::select(year, p_risk_national) %>% distinct
